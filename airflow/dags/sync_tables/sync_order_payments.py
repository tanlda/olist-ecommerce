...
def ecommerce_sync_order_payments_etl():
    @task(task_id="upload_landing")
    def upload_landing_order_payments():
        import polars as pl

        date_cursor = Variable.get("order_payments_date_cursor", default_var="2017-01-01")
        ...
        with conn.cursor() as cursor:
            query = f"""
                ...
                ON payments.order_id = selected.order_id
            """
        ...
        with NamedTemporaryFile() as tmpfile:
            ...
            object_name = f"landing/order_payments/order_payments.{date_cursor}.csv"
            client.fput_object(bucket_name, object_name, tmpfile.name)

        conn.close()

    def sync_lakehouse_order_payments():
        return SparkSubmitOperator(
            ...
            application_args=[
                ...
                "--date-cursor", "{{ var.value.get('order_payments_date_cursor', '2017-01-01') }}",
                "--test",
                "--ingest",
            ]
        )

    def sync_warehouse_order_payments():
        return SparkSubmitOperator(
            conn_id="spark",
    ...
    def update_cursor_order_payments():
        last_date_cursor = Variable.get("order_payments_date_cursor", default_var="2017-01-01")
        next_date_cursor = datetime.strptime(last_date_cursor, "%Y-%m-%d") + timedelta(days=1)
        Variable.set("order_payments_date_cursor", next_date_cursor.strftime("%Y-%m-%d"))

    upload_landing = upload_landing_order_payments()
    sync_lakehouse = sync_lakehouse_order_payments()
    sync_warehouse = sync_warehouse_order_payments()
    update_cursor = update_cursor_order_payments()

    upload_landing >> sync_lakehouse >> sync_warehouse >> update_cursor


ecommerce_sync_order_payments_etl()
